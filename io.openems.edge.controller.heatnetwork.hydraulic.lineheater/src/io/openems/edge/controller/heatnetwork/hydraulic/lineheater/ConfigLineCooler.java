...
@interface ConfigLineCooler {
    ...
    boolean useDecentralizedCooler() default false;

    @AttributeDefinition(name = "optional DecentralizedHeater", description = "If a DecentralizedCooler directly needs a communication")
    String decentralizedCoolerReference() default "DecentralizedCooler0";

    @AttributeDefinition(name = "ReactionType", description = "Should the LineHeater open, when the DecentralizedCooler "
            + "sends a Cool request or MORE Cool request")
    DecentralizedCoolerReactionType reactionType() default DecentralizedCoolerReactionType.NEED_COOL;
